...
class Thinning {
	...
	try{
	     ...
	     */
	    /* 例:コピーを行う処理 */
	    binarization(readImage, writeImage, w,h);
     ...
     */
    /* コピーメソッド */
    public static void binarization(BufferedImage readImage, BufferedImage writeImage, int w, int h){
	...
	int p1,p2=0,p3,p4,p5,p6,p7,p8,p9;
	boolean flag  = true;
	while (flag) {
	    flag = false;
	    for (int z = 0; z < 2; z++) {
		for (int y = 0; y < h; y++) {
			...
			if (z == 0) {
			    if (p2*p4*p6 == 0 && p4*p5*p8 == 0) count++;
			} else {
			    if (p2*p4*p8 == 0 && p2*p6*p8 == 0) count++;
			}
